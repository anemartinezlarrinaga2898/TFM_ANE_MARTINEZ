...
MutacionesLuad$V16 <- gsub("(-01A).*|(-01B).*","",MutacionesLuad$V16) #En este data.frame tengo a todos mis pacientes
colnames(MutacionesLuad) <- MutacionesLuad[1,] #ponemos los nombres en las columnas 
MutacionesLuad <- MutacionesLuad[-1,]
saveRDS(MutacionesLuad,"TotalMutacionesLuad.rds")

GrupoK3_Luad<- readRDS("K3_NMF_Luad.rds")
GrupoK3_Luad <- GrupoK3_Luad[order(GrupoK3_Luad$nmf_subtypes),]
GrupoK3_Luad <- GrupoK3_Luad[,-3]

PacientesLuad <- GrupoK3_Luad$Sample_ID #Nombres de los pacientes de LUAD
...
saveRDS(GenesMutadosInteres,"GenesMutadosPacientes_Luad.rds")
...
GenesMutadosInteres_PertenenciaCluster <- inner_join(GrupoK3_Luad,GenesMutadosInteres,by="Sample_ID")
...
ATM <- GenesMutadosInteres_PertenenciaCluster %>% filter(Hugo_Symbol=="ATM")
ATM <- ATM[order(ATM$nmf_subtypes),]
ATM <- ATM[!duplicated(ATM$Sample_ID),]
...
MatrizPerfilMutagenico <- matrix(0,nrow=length(HugoNames)+1,ncol=length(PacientesLuad))
...
MatrizPerfilMutagenico[1,] <- GrupoK3_Luad$nmf_subtypes


#Juntar todas las mutaciones en un unico data.frame 

Mutaciones <- rbind(TP53,STK11)
Mutaciones <- rbind(Mutaciones,ATM)
...
ComprobacionMatriz_ATM <- sum(as.numeric(MatrizPerfilMutagenico[4,])) #3
...
saveRDS(MatrizPerfilMutagenico,"MatrizPerfilMutagenico_Luad.rds")
...
MatrizPerfilMutagenico <- readRDS("MatrizPerfilMutagenico_Luad.rds")
...
PerfilMutagenico <- ggplot(data = Matriz_Longer_Pertenencia, 
  ...
  ggtitle(label = "Perfil Mutagenico Luad")
