...
directory <- setwd( "/Users/anemartinezlarrinaga/OneDrive/2_MASTER_Computacional/5_TFM/CODIGO/SCRIPTS_Y_DATOS/2_ScriptsPreProcesamiento/lusc/Datos/")
library(tidyverse)
...
DatosLusc_Manifiesto <- read.delim("~/OneDrive/2_MASTER_Computacional/5_TFM/CODIGO/SCRIPTS_Y_DATOS/1_ScriptsObtencionDeDatos/ManifiestoFinales/ManifiestoLusc_Completo_Unico.txt")
...
for (i in seq(3,495,1)){
  ...
  unionLusc <- full_join(sample,sampleNuevo,by="V1")
  sample <- unionLusc
}

#Colocamos el nombre de los genes en las filas
rownames(unionLusc) <- unionLusc$V1
unionLusc <- unionLusc[,-1]
...
saveRDS(unionLusc,"MatrizCuentasLusc.rds")
