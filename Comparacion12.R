...
#OBJETIVO:# COMPARACION 12: Grupo 2 de LUSC que es inmunogenico frente al grupo 1 de LUAD que tambien es imunogenico, pero sin tener en cuenta el GO TERM. 
#Trabajamos con toda la huella. 
...
GeneList_TG1 <- readRDS("DatosPartida/GSEA/GeneListFiltro3/GLF1_Luad.rds")
GeneList_TG2 <- readRDS("DatosPartida/GSEA/GeneListFiltro3/GLF2_Lusc.rds")
...
venn.diagram(
  ...
  category.names = c("Grupo 1 LUAD" , "Grupo 2 LUSC "),
  filename = "Comparacion_Grupo_1_LUAD_frente_a_Grupo_2_LUSC_HUELLA_3.png",
  output=TRUE,
  lwd = 2,
  lty = 1,
  col=c("#A3B2F7", '#B1F7A3'),
  fill = c(alpha("#A3B2F7",0.5), alpha('#B1F7A3',0.5)),
  cat.cex = 0.6,
  cat.fontface = "bold",
...
AnotacionesUnicos_TG1 <- getBM(attributes = c("ensembl_gene_id","hgnc_symbol"),
                               ...
                               values = Expresion_GenesUnicos_TG1$ensembl_gene_id,
                               mart = ensembl)
AnotacionesUnicos_TG2 <- getBM(attributes = c("ensembl_gene_id","hgnc_symbol"),
                               filters = "ensembl_gene_id",
                               values = Expresion_GenesUnicos_TG2$ensembl_gene_id,
                               mart = ensembl)
AnotacionesDuplicados <- getBM(attributes = c("ensembl_gene_id","hgnc_symbol"),
                               filters = "ensembl_gene_id",
                               values = DuplicadosTotales$ensembl_gene_id,
                               mart = ensembl)
...
saveRDS(Anotaciones_NivelesExpresion_TG1,"Comparacion12/Anotaciones_NivelesExpresion_TG1.rds")
saveRDS(Anotaciones_NivelesExpresion_TG2,"Comparacion12/Anotaciones_NivelesExpresion_TG2.rds")
saveRDS(Anotaciones_NivelesExpresion_Duplicados,"Comparacion12/Anotaciones_NivelesExpresion_Duplicados.rds")
