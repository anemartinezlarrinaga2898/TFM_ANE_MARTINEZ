...
RF <- readRDS("Analisis_Filtro3/Resultados_GSEA/Filtrado/AnalisisGSEA_Luad_1_Filtrado_Hallamarks.rds")
RT <- RF

RT <- RT[-1]
Condiciones <- c("C2","C5","C6","C7")
...
RF <- readRDS("Analisis_Filtro3/Resultados_GSEA/Filtrado/AnalisisGSEA_Luad_2_Filtrado_Hallamarks.rds")
RT <- RF
...
H<- ggplot(H, aes(x=NES, y=Description))+
  geom_point(aes(colour=p.adjust,size=Cuentas))+
  scale_color_gradientn(colours=rainbow(4), limits=c(0, 0.05))+
  labs(title="H Genes , Group 2 Luad, Filtrados")
H
...
RF <- readRDS("Analisis_Filtro3/Resultados_GSEA/Filtrado/AnalisisGSEA_Luad_3_Filtrado_Hallamarks.rds")
RT <- RF

RT <- RT[c(-1,-5)]
Condiciones <- c("C2","C5","C6")
i <- 1
